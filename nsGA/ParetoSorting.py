...
class ParetoSorting:
    def __init__(self, mover, new_population, number, mode, steam_or_not):
        translation_result = translation(new_population)
        self.objectives_result = ResultsOfObjectives(mover, translation_result, number, mode, steam_or_not)
        ...
        for j in range(len(self.pareto_sorting_result[0])):
            ...
            self.pareto_sorting_best.append(new_population[the_number])  # 帕累托最优解
            ...
            self.pareto_sorting_best_pei_result.append(self.objectives_result.pei_objective
                                                       [the_number])
            self.ele_waste_0.append(self.objectives_result.ele_waste_0[the_number])
            self.ele_waste_1.append(self.objectives_result.ele_waste_1[the_number])
            self.ele_waste_2.append(self.objectives_result.ele_waste_2[the_number])
            self.heat_waste.append(self.objectives_result.heat_waste[the_number])
            self.cold_waste.append(self.objectives_result.cold_waste[the_number])
...
class ReverseParetoSorting:
    def __init__(self, mover, new_population, number, mode, steam_or_not):
        ...
        self.ele_waste_0 = self.objectives_result.ele_waste_0
        self.ele_waste_1 = self.objectives_result.ele_waste_1
        self.ele_waste_2 = self.objectives_result.ele_waste_2
        self.heat_waste = self.objectives_result.heat_waste
        self.cold_waste = self.objectives_result.cold_waste
...
class ResultsOfObjectives:
    def __init__(self, mover, translation_result, number, mode, steam_or_not):
        self.cost_objective = []
        ...
        for i in range(len(translation_result)):
            ...
            if mover == 0:  # 燃气轮机
                objective_mover_0 = objective_function(temporary, number, mode, steam_or_not)
                self.cost_objective.append(objective_mover_0[0])
                self.emission_objective.append(objective_mover_0[1])
                self.pei_objective.append(objective_mover_0[2])
                self.ele_waste_0.append(objective_mover_0[3])
                self.ele_waste_1.append(objective_mover_0[4])
                self.ele_waste_2.append(objective_mover_0[5])
                self.heat_waste.append(objective_mover_0[6])
                self.cold_waste.append(objective_mover_0[7])
            else:  # 内燃机
                objective_function_mover_1 = Objective(temporary, number, mode, steam_or_not)
                self.cost_objective.append(objective_function_mover_1.cost)
                self.emission_objective.append(objective_function_mover_1.co2_emission)
                self.pei_objective.append(objective_function_mover_1.PER)
                self.ele_waste_0.append(objective_function_mover_1.ele_waste_0)
                self.ele_waste_1.append(objective_function_mover_1.ele_waste_1)
                self.ele_waste_2.append(objective_function_mover_1.ele_waste_2)
                self.heat_waste.append(objective_function_mover_1.heat_waste)
                self.cold_waste.append(objective_function_mover_1.cold_waste)
