...
class InternalCombustionEngine:
    ...
    def get_ele_out_through_heat_mode_boiler(self, heat_out):  # heat_out 为exhaust gas进入锅炉制热水+jacket water的热量
        constant = heat_out / self.nominal
        a = self.ice_database_one[6] * Parameters.effi_Boiler + self.ice_database_one[4]
        b = self.ice_database_one[7] * Parameters.effi_Boiler + self.ice_database_one[5] \
            - constant * self.ice_database_one[2]
        c = -constant * self.ice_database_one[3]
        delt = math.pow(b, 2) - 4 * a * c
        pl_1 = (-b + math.pow(delt, 0.5)) / (2 * a)
