...
class CHP:
    def __init__(self, temporary, number):
        gasturbine = GasTurbine(number, temporary)  # 为求H_CHP_out_max 和 C_CHP_out_max调整pl = 1
        ...
        if gasturbine.get_heat_out(Parameters.get_nominal_GasTurbine(temporary)) <= boiler.heat_in_max:
            boiler_heat_in = gasturbine.get_heat_out(Parameters.get_nominal_GasTurbine(temporary))
            # 进入余热锅炉的热量，但设计的结果一定是gasturbine.get_H_out(P.nominal_GasTurbine) >= boiler.heat_in_max #
        else:
            boiler_heat_in = boiler.heat_in_max

        self.H_out_max = boiler.get_H_out(boiler_heat_in)

        if boiler.get_H_out(boiler_heat_in) <= absorptionchiller.heat_in_max:
            absorptionchiller_heat_in = boiler.get_H_out(boiler_heat_in)
